/*
 * PlayRandomFileBufferedOnInterruptLowPower
 *
...
uint8_t sampleBuffer[bufferSize];
bool lastLoadedLowSide = false;
...
loop()
{
	...
	if (bufferPos >= bufferSize / 2) {
		if (!lastLoadedLowSide) {
			uint8_t samplesToRead = min(bufferSize / 2, samplesRemaining - bufferSize + bufferPos);
			uint8_t samplesRead = sampleFile->readByteArray(sampleBuffer, samplesToRead);
			if (Serial && samplesRead < samplesToRead) {
				Serial.print(samplesRead);
				Serial.print(" samples read, expecting ");
				Serial.println(samplesToRead);
			}
			lastLoadedLowSide = true;
		}
	} else {
		if (lastLoadedLowSide) {
			uint8_t samplesToRead = min(bufferSize / 2, samplesRemaining - bufferSize + bufferPos);
			uint8_t samplesRead = sampleFile->readByteArray(sampleBuffer + (bufferSize / 2), samplesToRead);
			if (Serial && samplesRead < samplesToRead) {
				Serial.print(samplesRead);
				Serial.print(" samples read, expecting ");
				Serial.println(samplesToRead);
			}
			lastLoadedLowSide = false;
		}
	}
}
...
wakeUpFromSleep()
{
	...
	//*** The datasheet says the chip takes a maximum of 10ms to wake from sleep
	//*** I like a small buffer personally.
	delay(12);
...
startPlayback()
{
	//*** First initialize any variables which need to be reset on new playback

	isPlaying = true;
	bufferPos = 0;
	samplesRemaining = sampleFile->getFileSize() - sampleFile->getFilePosition();

	uint8_t samplesToRead = min(bufferSize, samplesRemaining);
	uint8_t samplesRead = sampleFile->readByteArray(sampleBuffer, samplesToRead);
	if (Serial && samplesRead < samplesToRead) {
		Serial.print(samplesRead);
		Serial.print(" samples read, expecting ");
