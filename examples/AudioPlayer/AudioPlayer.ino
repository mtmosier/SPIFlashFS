...
uint16_t fileNum;
int speakerPin = 3;
const unsigned int stepDownCount = 10;



SPIFlashFS* fs;
SPIFlashFSFile* sampleFile;


volatile bool isPlaying = false;
volatile uint32_t sampleCount = 0;
uint8_t lastSample;
uint32_t sampleFileSize = 0;
...
{
	Serial.begin(115200);
	Serial.println("Beginning audio player test");
	Serial.println();
...
/*
 * Updated to use pin 3 instead of 11, and to read the data from a global
 * file object, in this case SPIFlashFSFile
 *
...
ISR(TIMER1_COMPA_vect) {
	if (sampleCount >= sampleFileSize) {
		if (sampleCount == sampleFileSize + (lastSample * 5)) {
			stopPlayback();
		} else {
	      // Ramp down to zero to reduce the click at the end of playback.
	      OCR2B = lastSample + ((sampleFileSize - sampleCount) / 5);
		}
	} else {
...
void startPlayback(/*unsigned char const *data, int length*/)
{
	...
	sampleFileSize = sampleFile->getFileSize();
	sampleFile->seek(sampleFileSize - 1);
	lastSample = sampleFile->readByte();
	sampleFile->seek(1);
	sampleCount = 0;
